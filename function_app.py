...
@app.event_hub_message_trigger(arg_name="azeventhub", event_hub_name="test-hub2",
                               connection="dt2005eventhub_RootManageSharedAccessKey_EVENTHUB") 
def eventhub_trigger(azeventhub: func.EventHubEvent):
    logging.info('Python EventHub trigger processed an event: %s',
                azeventhub.get_body().decode('utf-8'))

# Azure Portal에서 실습한 것: Event Hubs에 데이터가 수신될 때마다 함수가 호출되는 
# VSCode 에서 함수를 실행하는 것: 

# even hubs trigger를 여러 개 사용해서 쓰느냐, stream analytics를 쓰느냐
...
@app.event_hub_output(arg_name="event", event_hub_name="test-hub2", 
                      connection="dt2005eventhub_RootManageSharedAccessKey_EVENTHUB")
...
#     arg_name="event", event_hub_name="test-hub", connection="dt2005eventhub_RootManageSharedAccessKey_EVENTHUB"
# )
